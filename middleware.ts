...
export async function middleware(request: NextRequest) {
	...
	if (
		...
		if (restaurant?.slug) {
			return NextResponse.redirect(
				new URL(`/${restaurant.slug}/admin`, request.url),
			);
		}
	...
	if (user && !isAuthRoute(pathname)) {
		...
		if (
			(isCashierRoute && role === "waiter") ||
			(isWaiterRoute && role === "cashier")
		) {
			const segments = pathname.split("/").filter(Boolean);
			const slug = segments.length > 0 ? segments[0] : "";
			if (slug) {
	...
	// ── Subscription hard gate for tenant admin routes ──
	if (user && isAdminRoute(pathname) && !isTrialSafeRoute(pathname)) {
		const slug = pathname.split("/").filter(Boolean)[0];

		if (slug) {
			const { data: restaurant, error: restaurantError } = await supabase
				.from("restaurants")
				.select("slug, plan, trial_ends_at, subscription_status")
				.eq("slug", slug)
				.maybeSingle();
			...
			if (
				...
				return NextResponse.redirect(
					new URL(
						`/${slug}/admin/settings/billing?from=subscription-hard-gate`,
						request.url,
					),
				);
			}
